...
| GSS IOV and PRF (RFC 4121 / RFC 4402) | Typed HEADER/DATA/PADDING/TRAILER/SIGN_ONLY IOV wrapping, STREAM unwrap, confidentiality and integrity-only tokens, RFC 4402 full/partial-key PRF, AES-SHA1/AES-SHA2/Camellia vectors | `TestGoGSSIOVAgainstMIT` and `TestMITGSSIOVAgainstGo` use `/usr/bin/python3` `gssapi.raw.wrap_iov`/`unwrap_iov`; the binding has no raw PRF API, so PRF uses MIT vectors | `krb5/gssapi` IOV layout, malformed-input, stream, SIGN_ONLY, and MIT PRF-vector coverage |
