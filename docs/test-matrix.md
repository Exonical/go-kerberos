...
| PA-SPAKE (Edwards25519 group 1) | Go client + Go KDC unit coverage; MIT vector goldens | `TestMITClientSPAKEAgainstGoKDC` (real MIT `kinit`, trace asserts SPAKE response) | `TestGoClientSPAKEAgainstMITKDC` with `spake_preauth_groups = edwards25519` |
