...
| GSS channel bindings (RFC 1964 / RFC 4121) | RFC 1964 MD5 encoding, RFC 4121 `0x8003` Bnd placement, MS-KILE `KERB_AP_OPTIONS_CBT` assertion/checking, matching/mismatch/absent/zero-binding tolerance, and SPNEGO/IAKERB propagation | `TestGoGSSChannelBindingsAgainstMIT`, `TestMITGSSChannelBindingsAgainstGo` (Python GSSAPI linked to MIT) | `krb5/gssapi` unit coverage |
...
| MIT trace API (`KRB5_TRACE`, callback, and file trace) | Go client/KDC trace callbacks and MIT-compatible file formatting | No live MIT `t_trace.py` gate is enabled; the installed MIT fixture is reserved for protocol interoperability and does not expose a disposable trace assertion harness | Hermetic callback, environment, formatting, and AS/TGS emission tests |
...
| KRB-SAFE / KRB-PRIV application messages | Hermetic Go round trips with keyed checksums, encryption, addresses, timestamps, sequence validation, and optional replay-cache duplicate detection/expiry | No standalone MIT live gate; KRB-PRIV compatibility is exercised through the existing kpasswd message shape | Go appmsg APIs and kprop/kpasswd-compatible KRB-SAFE/KRB-PRIV encoding |
...
## Operational parity utilities

`krb5/klog` parses MIT `[logging]` destinations for `kdc`,
`admin_server`, and `default`, including `FILE=`, `FILE:`, `SYSLOG`,
`STDERR`, `CONSOLE`, and `DEVICE`. File-like destinations use MIT's
`Mon dd HH:MM:SS host program[pid](severity): message` layout; an unset
destination defaults to `SYSLOG` with the `AUTH` facility. KDC and kadm5
